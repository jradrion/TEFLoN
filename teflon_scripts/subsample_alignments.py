...
def worker(task_q, params):
    while True:
        try:
            ...
            #unpack parameters
            exePATH, qual, min_seqs = params
            for sample in samples:
                bamFILE = sample[0].replace(".bam",".subsmpl.bam")
                if not os.path.exists(bamFILE):
                    print "Creating",bamFILE
                    try:
                        fract=round(1/float(sample[2][4]/float(min_seqs)),3)
                        if fract < 1.0:
...
def subsample_alignments_portal(samples, exePATH, nProc, qual, cov):
    if cov == -1:
